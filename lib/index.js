...
const getDataStrategy = {
  ...
  viewport: () => {
    ...
    let width = win.innerWidth || el.clientWidth || body.clientWidth;
    let height = win.innerHeight || el.clientHeight || body.clientHeight;
...
const createTableRows = (dataToDisplay = [], dataLabel = "qa-info") => {
  let windowContent = dataToDisplay.reduce((accumulator, current) => {
...
const formatDataForConfluence = (dataToDisplay = []) => {
  let clipboardString = dataToDisplay.reduce((accumulator, current) => {
...
export default class QaInfobox {
  ...
  decodeCorrectKey(customKey) {
    ...
    keyObject = { keyCode, shiftKey, altKey, ctrlKey };
    return keyObject;
  }
  ...
  open() {
    if (this.el) {
      ...
      let dataToDisplay = this.collectDataToDisplay(getDataStrategy, this.customData);
  ...
  appendServerInfo(response) {
    let dataToDisplay = this.collectDataToDisplay(response);
    let rowsToAppend = createTableRows(dataToDisplay, "qa-serverinfo");
  ...
  createPopup(dataToDisplay = []) {
    ...
    let windowContent = createTableRows(dataToDisplay, "qa-info");
    let htmlTemplate = `<table class="qa-content">
     <tr><td colspan="2">
   ...
   */
  copyToClipboard() {
    let clipboard = this.el.querySelector(".qa-infobox-clipboard");
