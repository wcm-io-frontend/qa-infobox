...
const getDataStrategy = {
  ...
  os: () => navigator.userAgent.replace(/^.+?\(([^;)]+).*\).+$/, "$1"),
  monitor: () => `${screen.width} x ${screen.height}`,
  browser: () => `${navigator.appName} ${navigator.appVersion.replace(/^([\d.]+).*$/, "$1")}`,
...
export default class QaInfobox {
  ...
  constructor({
    ...
    this.defaultFieldsStrategy = requiredFields
      ? requiredFields.reduce((accumulator, fieldName) => {
          if (fieldName in getDataStrategy) {
            accumulator[fieldName] = getDataStrategy[fieldName];
          }
          return accumulator;
        }, {})
      : Object.assign({}, getDataStrategy);
