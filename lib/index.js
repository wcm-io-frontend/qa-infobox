...
const COPY_TO_CLIPBOARD_ID = `js-${CLASSNAME}__copytoclipboard`;
const CLOSE_ID = `js-${CLASSNAME}__close`;
const CLIPBOARD_ID = `js-${CLASSNAME}__clipboard`;
 ...
 *
 * @param {string} id   id which will be attached to topmost element; default
 *                      qa-infobox-{TIMESTAMP}
...
export default class QaInfobox {
  ...
  close(event) {
    ...
    if (this.el) {
      this.el.querySelector(`#${CLIPBOARD_ID}`).style.display = "none";
  ...
  createPopup(dataToDisplay = []) {
    ...
    const htmlTemplate = `<table class="qa-e-content">
     <tr><td colspan="2">
       <button id="${COPY_TO_CLIPBOARD_ID}" class="${CLASSNAME}__copytoclipboard">
         Copy To Clibboard as Confluence Markup
       </button>
       <button id="${CLOSE_ID}" class="${CLASSNAME}__close">
         X
       </button>
       <textarea id="${CLIPBOARD_ID}" class="${CLASSNAME}__clipboard"></textarea>
     </td></tr>
      ${windowContent}
      </table>`;
    ...
    setTimeout(() => {
      ...
      document.getElementById(COPY_TO_CLIPBOARD_ID).addEventListener("click", this.copyToClipboard.bind(this));
      document.getElementById(CLOSE_ID).addEventListener("click", this.close.bind(this), false);
   ...
   */
  copyToClipboard() {
    const clipboard = this.el.querySelector(`#${CLIPBOARD_ID}`);
